...
| 5.15 | Runtime `require('@/...')` makes progress commands untestable | `MedicineCommand`, `WeddingCommand`, `CraneCommand`, `DiplomatCommand`, `IntrigueCommand`, `SaboteurCommand`, `progress-card-manager.ts` | Next's bundler resolves the `@/` alias inside `require()`, so the game works — but Node's require (what vitest supplies) cannot, and could not load a `.ts` file even if it did. Every one of these commands is therefore untestable past its validation guards. `TaxationCommand` was converted 2026-07-26 (`f68ca93`); `EspionageCommand` and `GuildDuesCommand` were converted and covered during the theft-notification work (`5f80530`). Convert the rest **one at a time, running the suite after each**: several look like deliberate cycle-breakers (`MedicineCommand` → victory-conditions, `progress-card-manager` → progress-card-definitions), so a blind sweep risks introducing a circular import |
