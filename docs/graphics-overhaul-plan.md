...
> 🟠 **Implemented in the working tree 2026-07-12; user visual pass and commit pending.** Shared tabletop glyph/modal primitives and all active trade, city/knight, progress/resource, robber, timer/status, game-over, and board-prompt consumers are migrated. `GameStatus.tsx`, `GameIcon.tsx`, and `ColoredSvgIcon.tsx` are deleted; `CompactImprovementBar` moved out of the retired icon directory. Static gates, TypeScript, and 194/194 tests pass. The now-unreferenced `public/icons/` directory remains only because the environment's destructive-action reviewer rejected its deletion; no runtime source references it. Direct requests previously confirmed both `/` and the live `/board/flat` route compile and return HTTP 200. Multiplayer QA passed lifecycle synchronization, Debug grants (QA-001 retracted after the overlap fix), road/settlement/city/wall/knight construction (QA-002 retracted after the target fix), bank/domestic/Commercial Harbor trades, discard synchronization, Road Building, Merchant selection/cancel, Wedding, and Alchemy. The 2026-07-15 continuation found three remaining defects: QA-003, the narrow progress-card trigger immediately recloses for hands of three or fewer cards; QA-004, the fixed desktop rails/tray/debug layout overlaps and clips core play surfaces at 390×844; and QA-005, Wedding gifts incorrectly trigger the robber-style “Resources Stolen!” notification. Taxation/Treason remain legitimately gated until the first barbarian attack; Aqueduct, timer expiry, and game-over still need reachable coverage.
...
- ✅ `npm run test:run` — 31 files, 197/197 tests (Debug grant feedback, legal perimeter-road Knight validation, accessible Knight targets, responsive progress drawer, and Wedding transfer semantics included).
...
**iPad/Safari responsive resolution (2026-07-15, uncommitted):** viewports below `xl` now use a touch-first HUD with mutually exclusive Status, Log & Chat, Decks, and Debug drawers; the board retains the full dynamic viewport and the bottom tray wraps into two rows throughout the iPad range. `viewport-fit=cover`, `100dvh`, safe-area insets, contained modal/drawer scrolling, deliberate tap highlighting, and 44px primary trade/debug/tab targets cover Safari chrome and touch interaction. Live two-player DVCL validation confirmed no document overflow, visible turn actions, working 3- and 4-card progress shelves, bank/domestic trade modal fit, and cross-tab Wedding Gift Sent/Received copy without theft terminology.
