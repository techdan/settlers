...
export const VertexRenderer: React.FC<VertexRendererProps> = ({ vertex, knight, size, color, onClick, isValid, theme = 'flat', isMoving, onCancelMove, currentPlayerId, showCancelIcon, cancelIconTitle, onCancelIconClick, isSelectedForAction, highlightVariant = 'default', isPendingPlacement, onConfirmPlacement, onCancelPlacement }) => {
    ...
    return (
        <g transform={`translate(${pixel.x + offset.x}, ${pixel.y + offset.y})`} onClick={() => onClick?.(vertex.id)} className={showPointer ? "cursor-pointer" : ""}>
            ...
            {vertex.structure === 'settlement' && (
                theme === 'voxel' ? (
                    <g transform="translate(0, -2) scale(2)">
                        ...
                        {/* Left Face */}
                        <path d="M0 4 L-8 0 L-8 -10 L0 -6 Z" fill={playerColorFill || 'gray'} filter="brightness(0.7)" stroke="none" />
                        {/* Right Face */}
                        <path d="M0 4 L8 0 L8 -10 L0 -6 Z" fill={playerColorFill || 'gray'} filter="brightness(0.5)" stroke="none" />
                        {/* Roof Left */}
                        <path d="M0 -6 L-8 -10 L0 -16 Z" fill={playerColorFill || 'gray'} filter="brightness(1.1)" stroke="none" />
                        {/* Roof Right */}
                        <path d="M0 -6 L8 -10 L0 -16 Z" fill={playerColorFill || 'gray'} filter="brightness(0.9)" stroke="none" />
                    </g>
                ) : (
                    ...
                    <g transform="translate(0, -2) scale(2)">
                        ...
                        {vertex.hasCityWall && (
                            <g>
                                ...
                                <path d="M0 8 L-13 2 L-13 -4 L0 2 Z" fill="var(--color-structure-wall-base)" />
                                <path d="M0 8 L13 2 L13 -4 L0 2 Z" fill="var(--color-structure-wall-shadow)" />
                                {/* Wall Top Lip */}
                                <path d="M0 2 L-13 -4 L0 -10 L13 -4 Z" fill="var(--color-structure-wall-highlight)" />
                            </g>
                        )}
                        {/* 3D City - Base + Tower */}
                        {/* Base Left Face */}
                        <path d="M0 5 L-10 0 L-10 -8 L0 -3 Z" fill={playerColorFill || 'gray'} filter="brightness(0.7)" />
                        {/* Base Right Face */}
                        <path d="M0 5 L10 0 L10 -8 L0 -3 Z" fill={playerColorFill || 'gray'} filter="brightness(0.5)" />
                        {/* Base Top */}
                        <path d="M0 -3 L-10 -8 L0 -13 L10 -8 Z" fill={playerColorFill || 'gray'} filter="brightness(0.9)" />
                        {/* Tower (Back Right) */}
                        <g transform="translate(5, -5)">
                            {/* Tower Left Face */}
                            <path d="M0 0 L-5 -2.5 L-5 -12.5 L0 -10 Z" fill={playerColorFill || 'gray'} filter="brightness(0.8)" />
                            {/* Tower Right Face */}
                            <path d="M0 0 L5 -2.5 L5 -12.5 L0 -10 Z" fill={playerColorFill || 'gray'} filter="brightness(0.6)" />
                            {/* Tower Roof */}
                            <path d="M0 -10 L-5 -12.5 L0 -17.5 L5 -12.5 Z" fill={playerColorFill || 'gray'} filter="brightness(1.2)" />
                        </g>
                    </g>
                ) : (
                    <>
                        {vertex.hasCityWall && (
                            <rect
                                ...
                                height={36}
                                fill="none"
                                stroke="var(--color-structure-wall-highlight)"
                                strokeWidth={4}
                                rx={4}
                        ...
                        <svg x={-14} y={-14} width={28} height={28} viewBox="0 0 512 512" className="pointer-events-none" style={{ filter: `drop-shadow(0 2px 4px rgba(0,0,0,0.5))` }}>
                            ...
                            <path fill="var(--color-highlight-white)" d="M255.95 27.11L180.6 107.614l150.7 1.168-75.35-81.674h-.003zM25 109.895v68.01l19.412 25.99h71.06l19.528-26v-68h-14v15.995h-18v-15.994H89v15.995H71v-15.994H57v15.995H39v-15.994H25zm352 0v68l19.527 26h71.06L487 177.906v-68.01h-14v15.995h-18v-15.994h-14v15.995h-18v-15.994h-14v15.995h-18v-15.994h-14zm-176 15.877V260.89h110V126.63l-110-.857zm55 20.118c8 0 16 4 16 12v32h-32v-32c0-8 8-12 16-12zM41 221.897V484.89h78V221.897H41zm352 0V484.89h78V221.897h-78zM56 241.89c4 0 8 4 8 12v32H48v-32c0-8 4-12 8-12zm400 0c4 0 8 4 8 12v32h-16v-32c0-8 4-12 8-12zm-303 37v23h-16v183h87v-55c0-24 16-36 32-36s32 12 32 36v55h87v-183h-16v-23h-14v23h-18v-23h-14v23h-18v-23h-14v23h-18v-23h-14v23h-18v-23h-14v23h-18v-23h-14zm-49 43c4 0 8 4 8 12v32H96v-32c0-8 4-12 8-12zm72 0c8 0 16 4 16 12v32h-32v-32c0-8 8-12 16-12zm80 0c8 0 16 4 16 12v32h-32v-32c0-8 8-12 16-12zm80 0c8 0 16 4 16 12v32h-32v-32c0-8 8-12 16-12zm72 0c4 0 8 4 8 12v32h-16v-32c0-8 4-12 8-12zm-352 64c4 0 8 4 8 12v32H48v-32c0-8 4-12 8-12zm400 0c4 0 8 4 8 12v32h-16v-32c0-8 4-12 8-12z" />
                        </svg>
                    </>
                )
            )}
                    ...
                    <g transform="translate(0, -2) scale(2.2)">
                        ...
                        {vertex.hasCityWall && (
                            <g transform="scale(0.9)">
                                {/* Wall Base - Dark Brown */}
                                <path d="M0 9 L-14 3 L-14 -3 L0 3 Z" fill="var(--color-structure-wall-base)" />
                                <path d="M0 9 L14 3 L14 -3 L0 3 Z" fill="var(--color-structure-wall-shadow)" />
                                {/* Wall Top Lip */}
                                <path d="M0 3 L-14 -3 L0 -9 L14 -3 Z" fill="var(--color-structure-wall-highlight)" />
                            </g>
                        )}
                        {/* 3D Metropolis */}
                        <path d="M0 6 L-12 1 L-12 -6 L0 -1 Z" fill={playerColorFill || 'gray'} filter="brightness(0.7)" />
                        <path d="M0 6 L12 1 L12 -6 L0 -1 Z" fill={playerColorFill || 'gray'} filter="brightness(0.5)" />
                        <path d="M0 -1 L-12 -6 L0 -11 L12 -6 Z" fill={playerColorFill || 'gray'} filter="brightness(0.9)" />
                        <g transform="translate(0, -11)">
                            <path d="M0 0 L-8 -3 L-8 -8 L0 -5 Z" fill={playerColorFill || 'gray'} filter="brightness(0.8)" />
                            <path d="M0 0 L8 -3 L8 -8 L0 -5 Z" fill={playerColorFill || 'gray'} filter="brightness(0.6)" />
                            <path d="M0 -5 L-8 -8 L0 -11 L8 -8 Z" fill={playerColorFill || 'gray'} filter="brightness(1.0)" />
                        </g>
                        <g transform="translate(0, -22)">
                            <path d="M0 0 L-5 -2 L-5 -6 L0 -4 Z" fill="var(--color-structure-metropolis)" filter="brightness(0.9)" />
                            <path d="M0 0 L5 -2 L5 -6 L0 -4 Z" fill="var(--color-structure-metropolis)" filter="brightness(0.7)" />
                            <path d="M0 -4 L-5 -6 L0 -8 L5 -6 Z" fill="var(--color-structure-metropolis)" filter="brightness(1.2)" />
                            <path d="M0 -8 L-2 -10 L0 -14 L2 -10 Z" fill="var(--color-structure-metropolis)" filter="brightness(1.3)" />
                        </g>
                    </g>
                ) : (
                    <>
            ...
            {knight && knightStyle && (
                theme === 'voxel' ? (
                    <VoxelKnight knight={knight} color={playerColorFill || 'gray'} />
                ) : (
                    <g>
